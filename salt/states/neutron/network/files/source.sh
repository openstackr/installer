...
export OS_AUTH_URL=http://{{ salt['publish.publish']('function:controller','grains.get','ipv4','grain').values()[0][1] }}:35357/v2.0
export OS_SERVICE_TOKEN={{ keystone.conn_token }}
export OS_SERVICE_ENDPOINT=http://{{ salt['publish.publish']('function:controller','grains.get','ipv4','grain').values()[0][1] }}:35357/v2.0
